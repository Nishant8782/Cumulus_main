...
const GoogleDrivePicker = ({ folderId, fetchFiles, setShowLoadingIndicator ,setGoogleDrive }) => {
  ...
  return (
    <div>
      <div className="flex border border-slate-500 rounded-lg p-1 md:p-2 text-xs md:text-sm font-semibold cursor-pointer text-gray-500" onClick={handleAuth}>
        <img src={google} className="h-6 w-6 mr-2 mt-1 ml-1" />
        <button >
          <h1 className="mt-1 md:mt-0 md:inline hidden">Google Drive</h1>
        </button>
      </div>
