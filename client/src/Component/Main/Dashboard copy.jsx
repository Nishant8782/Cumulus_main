...
const [fileNames, setFileNames] = useState({});
  ...
  const startFileUpload = (file) => {
    ...
    const customFileName = `${file.name.split(".")[0]}_${Date.now()}.${fileExtension}`; // Custom name logic
  
    simulateUpload({ id: customFileName, name: file.name }); // Pass the file to simulateUpload
    setUploadQueue((prevQueue) =>
      prevQueue.map((item) =>
        item.id === customFileName
          ? { ...item, isUploading: true } // Set isUploading to true when starting the upload
          : item
      )
    );
  };
...
const handleFileUpload = (input) => {
  ...
  if (files && files.length > 0) {
    ...
    fileArray.forEach((file) => {
      const customName = editFile?.name || file.name;
      const fileExtension = file.name.split(".").pop();
  ...
  const handleSaveEdit = () => {
    ...
    setSelectedFile((prevSelected) => {
      if (!prevSelected.some((file) => file.id === editFile.id)) {
        console.warn("File not found in selectedFile array!");
        return prevSelected;
      }
      return prevSelected.map((file) =>
        file.id === editFile.id
          ? { ...file, name: updatedFileName, editedName: updatedFileName }
          : file
      );
    ...
    setTimeout(() => {
      console.log("Updated Selected Files After State Update:", selectedFile);
    }, 100);
  };
  ...
  const uploadFileToServer = async () => {
    console.log("Selected Files Before Upload: inside uploadFileToServer", selectedFile);
    ...
    try {
      ...
      const uploadPromises = selectedFile.map(async (fileObj) => {
        ...
        // ✅ Get updated file name from `files` array
        const updatedFile = files.find((f) => f.id === fileObj.id);
  
        // ✅ Ensure edited name is used for upload
        const finalFileName = updatedFile?.editedName || updatedFile?.name || fileObj.name;
        ...
        formData.append("custom_file_name", finalFileName); // ✅ Send edited name to server
      ...
      alert("A file with the same name is already present.");
    } finally {
      hideLoading();
    }
  };
  ...
  return (
    <div className=" px-4  bg-white">
      ...
      {viewMode === "list" ? (
        <>
          <div className="grid grid-cols-1 gap-4 md:hidden p-1 max-h-[50vh]  ">
                            ...
                            {file.access_details && file.access_details.length > 0 ? (
                              <div className="flex items-center gap-2">
                                ...
                                {file.access_details.length > 1 && (
                                  <span className="px-2 py-1 bg-slate-100 text-gray-700 rounded-md text-xs sm:text-sm">
                                    +{file.access_details.length - 1}
                                  </span>
                                )}
                              </div>
      ...
      {isUploading && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 mx-2 rounded shadow-lg max-w-lg w-full">
            <div className="flex justify-between items-center mb-4">
  ...
  return (
    <div key={file.id} className="flex justify-between items-center">
      <div className="w-3/4">
        ...
        {editing ? (inputFileName ? inputFileName : file.name) : file.name}
        </p>
...
<div className="flex justify-between gap-2 my-2">
            ...
            <div className="flex gap-x-6" onClick={() => {
                 setSelectedFile(null);
      setSelectedFolderId("");
...
>
  <div className="flex gap-x-6">
    ...
    />
              ...
              <button
                onClick={() => clear()}
                className="hidden md:inline border-dashed border-2 border-blue-500 text-blue-700 px-4 py-2 rounded hover:bg-blue-500 hover:text-white"
              >
                Cancel
              </button>
       ...
       {folderId === 0 || folderId === "0" && (
            <div className="mt-2">
              ...
              <button
                 onClick={() => addFolder()}
            className={`mt-4 px-4 py-2 rounded-md text-white w-full bg-[#0067FF]`}
          >
        Create Folder
          </button>
                </div>
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg max-w-sm w-full m-2">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => {setEditFile(false);
               setIsUploading(true);}
              }>
                <X className="w-5 h-5 text-gray-700 hover:text-red-500" />
              </button>
