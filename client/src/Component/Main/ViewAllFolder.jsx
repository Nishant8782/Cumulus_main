...
import { Trash, PencilLine, EllipsisVertical, Check, X, Plus } from "lucide-react"
import e from 'cors';


function ViewAllFolder({ onFolderSelect }) {
...
<div className='flex justify-between pb-2'>
        ...
        <button
          ...
          ><Plus className='mr-2 ' />Add Folder</button>
        </div>
      </div>
            ...
            {folders.map((folder, folderindex) => (
              <tr className='border-b-2 text-left' key={folder.id}>
                <td className='p-4  min-w-20'>
                  ...
                  <div 
                  ref={(el) => (dropdownRef.current[folderindex + 1] = el)}
              ...
              <div className='p-4 '>
                ...
                <div className='flex gap-x-2'>
                  <input
                    ...
                    value={editFolderName}
                    onChange={(e) => setEditFolderName(e.target.value)}
                    className={`${editable === folder.id ? 'inline outline-blue-600 p-2 border-2 text-gray-700 font-semibold rounded-lg' : 'hidden'}`}
                  />
                  <div className={`${editable === folder.id ? 'flex ' : 'hidden'}`}>
